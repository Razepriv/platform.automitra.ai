...
export async function syncOrganizationPhoneNumbers(
  ...
  try {
    ...
    // Fetch phone numbers from Exotel API
    let exotelNumbers: any[] = [];
    try {
      exotelNumbers = await exotelClient.getPhoneNumbers();
      console.log(`[Phone Sync] Fetched ${exotelNumbers.length} numbers from Exotel for org ${organizationId}`);
    } catch (error: any) {
      console.warn(`[Phone Sync] Could not fetch from Exotel API for org ${organizationId}:`, error.message);
      // Continue with configured numbers from env
    }

    // Also check configured numbers from environment
    const configuredNumbers: Array<{
      phoneNumber: string | undefined;
      appId: string | undefined;
      friendlyName: string;
    }> = [];

    if (process.env.EXOTEL_PHONE_NUMBER) {
      configuredNumbers.push({
    ...
    if (process.env.EXOTEL_PHONE_NUMBER_2) {
      configuredNumbers.push({
        phoneNumber: process.env.EXOTEL_PHONE_NUMBER_2,
        appId: process.env.EXOTEL_OUTBOUND_APP_ID_2,
        friendlyName: 'Exotel Voice Number 2',
      });
    }

    // Process Exotel API numbers
    for (const exotelNumber of exotelNumbers) {
      try {
        const phoneData: InsertPhoneNumber = {
          ...
          exotelStatus: exotelNumber.Status || 'active',
          capabilities: {
            voice: true,
            sms: true,
          },
          status: 'active',
          exotelConfig: {
            VoiceUrl: exotelNumber.VoiceUrl,
            VoiceMethod: exotelNumber.VoiceMethod,
            SmsUrl: exotelNumber.SmsUrl,
            SmsMethod: exotelNumber.SmsMethod,
            StatusCallback: exotelNumber.StatusCallback,
            DateCreated: exotelNumber.DateCreated,
            DateUpdated: exotelNumber.DateUpdated,
          },
        };
        ...
        if (!existingNumber) {
          ...
          stats.created++;
          console.log(`[Phone Sync] ✅ Created: ${exotelNumber.PhoneNumber} for org ${organizationId}`);
          if (emitPhoneNumberCreated) {
            emitPhoneNumberCreated(organizationId, created);
          }
        } else {
          // Update existing number with latest data
          ...
          if (updated) {
            stats.updated++;
            console.log(`[Phone Sync] 🔄 Updated: ${exotelNumber.PhoneNumber} for org ${organizationId}`);
        ...
        console.error(`[Phone Sync] ❌ Error processing Exotel number ${exotelNumber.PhoneNumber}:`, error.message);
        stats.errors++;
      }
    }

    // Process configured numbers from environment
    for (const configNum of configuredNumbers) {
      if (!configNum.phoneNumber) continue;
      ...
      try {
        ...
        if (!existingNumber) {
          const created = await storage.createPhoneNumber(phoneData);
          stats.created++;
          console.log(`[Phone Sync] ✅ Created configured number: ${configNum.phoneNumber} for org ${organizationId}`);
          if (emitPhoneNumberCreated) {
            emitPhoneNumberCreated(organizationId, created);
          }
        } else {
          // Update if needed
          if (existingNumber.exotelSid !== configNum.appId || existingNumber.friendlyName !== configNum.friendlyName) {
            const updated = await storage.updatePhoneNumber(existingNumber.id, organizationId, {
              exotelSid: configNum.appId || undefined,
              friendlyName: configNum.friendlyName,
            });
            stats.updated++;
            console.log(`[Phone Sync] 🔄 Updated configured number: ${configNum.phoneNumber} for org ${organizationId}`);
            if (emitPhoneNumberUpdate && updated) {
              emitPhoneNumberUpdate(organizationId, updated);
            }
          }
        }
      } catch (error: any) {
        console.error(`[Phone Sync] ❌ Error processing configured number ${configNum.phoneNumber}:`, error.message);
        stats.errors++;
      }
    }
